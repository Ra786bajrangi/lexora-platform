...
const UserDashboardPage = () => {
  ...
  // Fetch blogs when user or page changes
  useEffect(() => {
    const fetchUserBlogs = async () => {
      ...
      try {
        console.log("Making request to /api/blogs/my-blogs");
  
        const res = await axios.get(`http://localhost:5000/api/blogs/my-blogs?page=${pagination.currentPage}`, {
          headers: {
...
return (
  <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-100 transition-all duration-500">
    <header className="bg-white shadow-md">
      <div className="max-w-7xl mx-auto py-6 px-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-4">
          <span className="text-blue-800 font-semibold text-lg animate-typing overflow-hidden whitespace-nowrap border-r-4 border-blue-500 pr-2">
            Welcome, {user?.username || 'User'}
          </span>
          <Link
            to="/create-blog"
            className="px-4 py-2 text-sm font-semibold text-white bg-gradient-to-r from-purple-500 to-pink-500 rounded-md shadow hover:shadow-lg transition-all duration-300"
          >
            ✍️ New Post
          </Link>
        </div>
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {pagination.totalBlogs > 0 && (
          <span className="text-sm text-gray-600">
            ...
            {Math.min(pagination.currentPage * 10, pagination.totalBlogs)} of{' '}
